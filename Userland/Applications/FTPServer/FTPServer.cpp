...
{
    auto maybe_server = Core::TCPServer::try_create();
    if (maybe_server.is_error()) {
        return;
    }
    m_server = maybe_server.value();
    IPv4Address address = {};

    if (m_server->listen(address, m_port).is_error()) {
        log(String::formatted("INFO SERVER: Listening on {}:{} failed, restart to try again\n", address, m_port));
    }

    m_server->on_ready_to_accept = [this] {
        auto try_accept = on_ready_to_accept();
        if (try_accept.is_error()) {
            log("INFO SERVER: Error during client accept\n\n");
        }
...
ErrorOr<void> FTPServer::on_ready_to_accept()
{
    ...
    auto client_socket = TRY(m_server->accept());
    TRY(client_socket->set_blocking(true));

    auto client_thread = Threading::Thread::construct([&, this] {
        // Having this eventloop only to write to the text area feels dumb...
        ...
        client->on_data_transfer_start = [this](u32 id, String path) {
            ...
            m_transfer_table->model()->invalidate();
        };
    ...
    m_threads.append(move(client_thread));
